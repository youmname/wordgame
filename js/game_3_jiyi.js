...
(function() {
    ...
    // 输出到全局对象
    window.game_3WordDataLoader = game_3WordDataLoader;
    ...
    const WordBoard = {
        ...
        createCards(wordPairs) {
            ...
            for (let row = 0; row < this.rows; row++) {
                for (let col = 0; col < this.columns; col++) {
                    if (index < shuffledCards.length) {
                        ...
                        const cardElement = this.createCardElement(cardData, row, col);
            ...
            // 初始化ImprovedPathFinder
            if (window.ImprovedPathFinder) {
                window.ImprovedPathFinder.init(Math.max(this.rows, this.columns), document.querySelectorAll('.card'));
            }
        },
        ...
        createCardElement(cardData, row, col) {
            ...
            if (cardData.type === 'word') {
                content = `<div class="card-word">${cardData.content}</div>`;
            } else {
                content = `<div class="card-meaning">${cardData.content}</div>`;
            }
        ...
        /**
         * 选择卡片
         * @param {HTMLElement} card - 被选择的卡片元素
         */
        selectCard(card) {
            // 如果游戏已结束、卡片已匹配或动画进行中，则忽略点击
            if (window.WordGame && window.WordGame.isGameOver) return;
            if (card.classList.contains('matched')) return;
            if (this.animationInProgress) return;
            
            // 如果点击的是已选择的卡片，则忽略
            if (card === this.firstSelection) return;
        ...
        /**
         * 翻转卡片
         * @param {HTMLElement} card - 要翻转的卡片
         * @param {boolean} isFaceUp - 是否正面朝上
         */
        flipCard(card, isFaceUp) {
		...
		showHint() {
			...
			if (hintPair) {
				...
				allCards.forEach(card => {
					this.flipCard(card, true); // 翻转显示卡片正面
				});
				...
				// 延时2秒后将所有卡片翻转回去，但保持高亮
				setTimeout(() => {
					// 将所有卡片翻转回去
					allCards.forEach(card => {
						// 如果不是高亮的提示卡片，则翻转回去
						if (!card.classList.contains('hint')) {
							this.flipCard(card, false); // 翻转回卡片背面
						}
					});
					...
					setTimeout(() => {
						...
						// 将高亮的卡片也翻转回去
						if (hintPair.card1.element) this.flipCard(hintPair.card1.element, false);
						if (hintPair.card2.element) this.flipCard(hintPair.card2.element, false);
					}, 3000); // 3秒后移除高亮并翻转回去
				}, 2000); // 2秒后翻转非高亮卡片
    ...
    const GameCore = {
        ...
        async startGame() {
            ...
            if (isWin) {
                ...
                console.log(`[记忆游戏 - 积分计算] 基础分: ${baseScore}, 时间奖励: ${timeBonus}, 连击奖励: ${comboBonus}, 总得分: ${this.totalScore}`);
            ...
            if (isWin && this.currentLevelId && this.currentChapterOrderNum !== null && this.currentChapterOrderNum !== undefined) {
                ...
                console.log(`[记忆游戏] 准备更新进度: levelId=${this.currentLevelId}, completedOrderNum=${this.currentChapterOrderNum}, totalScore=${this.totalScore}`);
                ...
                .then(data => {
                    if (data.success) {
                        console.log(`[记忆游戏] 用户进度更新成功！获得积分: ${data.pointsEarned}`);
                        ...
                        console.error('[记忆游戏] 更新用户进度失败:', data.message);
                    }
                })
                .catch(error => {
                    console.error('[记忆游戏] 调用进度更新接口时出错:', error);
                });
            } else if (isWin) {
                console.log('[记忆游戏] 胜利，但缺少 levelId 或 chapterOrderNum，跳过进度更新 (可能是随机模式或导入模式?)');
            }
            // --- 结束新增代码 ---
        },
        ...
        async nextLevel() {
            ...
            if (userType === 'guest' && this.currentChapterOrderNum !== undefined && this.currentChapterOrderNum !== null) {
                ...
                if (!isNaN(currentNum) && currentNum >= guestLimit) {
                    console.log(`[记忆游戏] 游客达到限制 (关卡 ${currentNum})，阻止进入下一关。`);
            ...
            if (!this.currentLevelId || this.currentChapterOrderNum === undefined || this.currentChapterOrderNum === null) {
                console.error('[记忆游戏] 缺少 currentLevelId 或 currentChapterOrderNum 无法进入下一关');
            ...
            const currentNum = parseInt(this.currentChapterOrderNum, 10);
            if (isNaN(currentNum)) {
                 console.error('[记忆游戏] currentChapterOrderNum 无效:', this.currentChapterOrderNum);
            ...
            console.log(`[记忆游戏] 尝试构造并检查下一章节: ${predictedChapterId}`);

            const authToken = localStorage.getItem('authToken');
            if (!authToken) {
                console.error('[记忆游戏] 未找到认证令牌');
                alert('请先登录！');
            ...
            try {
                ...
                if (response.ok) {
                    // 章节存在，构建跳转 URL
                    console.log(`[记忆游戏] 找到下一章节 ${predictedChapterId}，准备跳转...`);
                    // 假设所有记忆游戏都使用 game_3_jiyi.html (修正：使用 category 和 chapter 参数名)
                    // const nextLevelUrl = `game_3_jiyi.html?levelId=${encodeURIComponent(this.currentLevelId)}&chapterId=${encodeURIComponent(predictedChapterId)}`;
                    const nextLevelUrl = `game_3_jiyi.html?category=${encodeURIComponent(this.currentLevelId)}&chapter=${encodeURIComponent(predictedChapterId)}&mode=normal`; // 添加 mode=normal
                    window.location.href = nextLevelUrl;

                } else if (response.status === 404) {
                    // 章节不存在，说明已经是最后一关
                    console.log(`[记忆游戏] 未找到章节 ${predictedChapterId}，判定为最后一关。`);
                     Swal.fire({
                        title: '恭喜通关！',
                        text: `您已完成 "${this.currentLevelId}" 级别的所有记忆关卡！`,
                        icon: 'success',
                    ...
                    // 其他错误
                    console.error(`[记忆游戏] 检查下一关时发生错误，状态码: ${response.status}`);
                ...
                WordUtils.LoadingManager.hide(); // 隐藏加载提示
                console.error('[记忆游戏] 请求下一关信息时网络或处理错误:', error);
