        ...
        async nextLevel() {
            ...
            try {
                ...
                if (data.success && data.nextChapter) {
                    ...
                    // 构造下一关的URL (假设游戏类型不变，仍是 game_3_jiyi.html)
                    const nextChapterUrl = `game_3_jiyi.html?chapter=${encodeURIComponent(nextChapter.id)}&category=${encodeURIComponent(this.currentLevelId)}&chapterName=${encodeURIComponent(nextChapter.name)}&mode=normal`;
