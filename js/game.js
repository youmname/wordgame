...
const WordGame = {
    ...
    startLevel(chapter) {
        ...
        // 从数据加载器获取单词数据
        WordDataLoader.loadChapterWords(chapter).then(wordPairs => {
            if (!wordPairs || wordPairs.length < 2) {
                console.error(`[startLevel] 章节 ${chapter} 单词数量不足:`, wordPairs);
                WordUtils.ErrorManager.showToast(`章节 ${chapter} 单词数量不足，无法开始游戏`);
                WordUtils.LoadingManager.hide();
            ...
            // 打乱顺序并限制数量
            let shuffledPairs = WordUtils.shuffle(wordPairs);
            
            // 限制最大对数
            if (shuffledPairs.length > WordConfig.BOARD.MAX_PAIRS) {
                console.log(`[startLevel] 限制单词数量从 ${shuffledPairs.length} 到 ${WordConfig.BOARD.MAX_PAIRS}`);
                shuffledPairs = shuffledPairs.slice(0, WordConfig.BOARD.MAX_PAIRS);
            }
            
            // 保存单词对
            this.wordPairs = shuffledPairs;
            console.log(`[startLevel] 加载了 ${this.wordPairs.length} 对单词`);
            ...
            console.error(`[startLevel] 加载章节 ${chapter} 单词失败:`, error);
            WordUtils.ErrorManager.showToast(`加载单词失败: ${error.message}`);
            WordUtils.LoadingManager.hide();
            
            // 尝试使用备选数据
            console.log('[startLevel] 尝试使用示例数据作为备选');
            const samplePairs = WordUtils.parseCustomInput(WordConfig.SAMPLE_DATA);
            
            if (samplePairs && samplePairs.length >= 4) {
                // 限制数量并打乱顺序
                const limitedPairs = samplePairs.slice(0, WordConfig.BOARD.MAX_PAIRS);
                this.wordPairs = WordUtils.shuffle(limitedPairs);
